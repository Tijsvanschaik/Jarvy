import { z } from "zod";
import type { AidenToolResult, AidenToolSpec } from "../shared/types";
...
export type ToolDefinition<TArgs extends z.ZodType = z.ZodType> = {
  name: string;
  args: TArgs;
  result?: z.ZodType<AidenToolResult>;
  parameters: Record<string, unknown>;
  description: string | (() => string);
  timeoutMs?: number;
  exposed?: boolean;
  handler: (args: any, context: ToolContext) => Promise<AidenToolResult> | AidenToolResult;
};
...
export class ToolHost {
  ...
  async invoke(name: string, rawArgs: unknown): Promise<AidenToolResult> {
...
export function toolError(code: ToolInvocationErrorCode, error: string, details?: Record<string, unknown>): AidenToolResult {

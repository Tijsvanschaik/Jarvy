...
export function OperatorPanel() {
  ...
  useEffect(() => {
    void window.ricky.getOpsState().then((next) => {
      setState(next);
      setBlock(next.block);
    });
    return window.ricky.onOpsState((next) => {
      setState(next);
  ...
  return (
    <main className="operator-shell">
      <header>
        <div>
          <span className="operator-kicker">Ricky operations</span>
        ...
        <button
          ...
          onClick={async () => {
            setBusy(true);
            try {
              setState(await window.ricky.hardCloseSession({ reason: "operator" }));
            } finally {
              setBusy(false);
            }
          }}
        ...
        <div>
          ...
          <button
            ...
            onClick={async () => {
              setBusy(true);
              try {
                setState(await window.ricky.setOpsBlock({ block }));
